...
/****************************** Common functions ******************************/
	...
	private final static byte PADDING = 0x00;
/***************************** SEED round function ****************************/
...
// reorder for little endian	  
	...
	public static byte[] addPadding(byte[] source, int blockSize) {
		int cnt = source.length % blockSize;
		byte[] paddingResult = null;
		
		if(cnt != 0) {
			paddingResult = new byte[source.length + (blockSize - cnt)];
			System.arraycopy(source, 0, paddingResult, 0, source.length);
			// 패딩해야 할 갯수 - 1 (마지막을 제외)까지 0x00 값을 추가한다.
			int addPaddingCnt = blockSize - cnt;
			for(int i=0;i<addPaddingCnt;i++) {
				paddingResult[source.length + i] = PADDING;
			}
